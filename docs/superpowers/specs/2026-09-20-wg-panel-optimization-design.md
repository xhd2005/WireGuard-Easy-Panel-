...
`select_client_ip`（`wg.sh:1065-1077`）用 `cut -d "." -f 4 | cut -d "/" -f 1` 从 `AllowedIPs` 抽末段，依赖 IPv6 地址恰好追加在同一行；上限判断 `[[ "$octet" -eq 255 ]]` 边界脆弱。

**修复**：改用 `grep -oE 'AllowedIPs = 10\.7\.0\.[0-9]+/32'` 精确提取，`octet` 显式校验 `2..254`。
...
6. **`wg.sh` 与面板对齐** — `# DNS` 注释行写入、性能项落地（5.2–5.5）、面板与菜单并存一致性复查
...
阶段 2 的 fixture 依赖已由阶段 0 消解（13.3），两者现在在同一份计划里按顺序排。当前目录还不是 git 仓库，所以本 spec 无法提交版本控制 —— 这正是阶段 0 第一件事。
