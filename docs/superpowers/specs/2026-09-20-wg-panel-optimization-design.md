...
- 现网事实与**代价**：目标机 WG 跑在 **UDP 53**，隧道本身工作正常（2.69 GiB 下行、握手新鲜）。但 v2 遗留的疑问"为什么 `127.0.0.53:53` 上没有 resolved 的 stub"已经查明，而且结论是**这个选择搞坏了这台机器的 Docker**：
  1. WG 绑定的是 `0.0.0.0:53`，通配地址覆盖了 `127.0.0.53`，于是 `systemd-resolved` 的 stub **绑不上自己的 53 端口** —— 日志直接写着 `Another process is already listening on UDP socket 127.0.0.53:53`
  2. `dockerd` 早于这次改动启动、把 DNS 服务器缓存成了 `127.0.0.53`，如今每次镜像解析都打到这个没人应答的地址：`lookup registry-1.docker.io on 127.0.0.53:53: i/o timeout`
  3. 后果：**这台机器当前 `docker pull` 任何镜像都失败**。业务容器还在跑所以没被察觉（容器自己的 DNS 走 `127.0.0.11` 嵌入服务器，不依赖宿主机那次查询），但一旦要更新镜像、加服务、或 1Panel 拉组件就会卡住

  因此端口建议要改写：**在跑 Docker 的机器上，UDP 53 是一个会牺牲 `docker pull` 的选项，不该被当作"已验证可行"而推广**。README 里必须写明这个连带效应；端口选择仍然回到"逐个 `ss` 实测 + 不预设推荐值"。
...
> **v3 修正**：原标题写的是"本机 Windows 即可运行"，实测证伪 —— 开发机与目标服务器都**没有 Go 工具链**。计划的 Docker 路线也在第一步就断了：目标机 `docker pull` 因 4.5 所述的 DNS 问题完全不可用。最终落地方式是往服务器 `/tmp` 解一份官方 Go 包（`golang.google.cn/dl`，71 MB，`GOTOOLCHAIN=local`），跑完删除 —— 不装到系统里、不留镜像、不碰 `/etc`。这条路径应当固化成 `scripts/test-in-vm.sh`，否则每次都要重新摸索。
